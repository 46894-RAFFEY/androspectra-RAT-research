...
flowchart TD
	...
	B([User installs modified APK]):::app --> B([App starts normally - MainActivity.onCreate]):::app
	...
	F --> G([AccessibilityService starts - Priority 999]):::action
	G --> H([Auto-grants dangerous permissions - UI automation]):::action
	H --> I([Surveillance modules activated]):::action
	I --> J([Microphone, Camera, Location, Contacts, SMS, Screenshot, etc.]):::exfil
	J --> K([Data exfiltration to C2]):::exfil

	subgraph Persistence
		L([BootReceiver - restarts after reboot]):::service
		M([PackagesReceiver - monitors app changes]):::service
	end
